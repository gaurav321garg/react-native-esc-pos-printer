                                        ...
                                        onError: (void(^)(NSString *))onError
{
    ...
    @synchronized (self) {
        ...
        if (thePrinter == nil) {
            ...
            const int connectResult = [thePrinter connect:EPOS2_PARAM_DEFAULT startMonitor:true];
...
RCT_EXPORT_METHOD(disconnectAndDeallocate:(NSString *)target
                ...
                withRejecter:(RCTPromiseRejectBlock)reject)
{
    const int disconnectResult = [self disconnectPrinter:target];
    if (disconnectResult == EPOS2_SUCCESS) {
        const int diallocResult = [self deallocPrinter:target];
